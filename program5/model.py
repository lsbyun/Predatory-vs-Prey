...
def mouse_click(x,y):
    if selected_object != '':
        if selected_object == 'Remove':
            ...
            string = selected_object + '({},{})'.format(x,y)
            simulton.add(eval(string))
...
def display_all():
    ...
    controller.the_progress.config(text=str(len(simulton))+" simultons/"+str(cycle_count)+" cycles")
